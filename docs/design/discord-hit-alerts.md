# Discord alerts for confirmed hits

Status: approved design. No production behavior is changed by this design branch.
...
## Ticket graph and publication manifest

One medium-sized vertical ticket: `Add private Discord hit alerts with game-only images`.
Scope: settings, UI, trigger integration, bounded worker/helper, capture, HTTP, tests, README, and source/frozen verification.
There are no ticket-to-ticket blocking edges. Attach it as a native sub-issue of the parent specification.
The design-artifact PR is a source reference, not a blocker to merge here. Implementation must carry accepted design files into its PR if they are not on main.

Publish these exact artifacts in `dusa97/MapleTapperLooker`:

- Parent issue: `[Spec] Discord hit alerts with game-only images`.
- Implementation sub-issue: `Add private Discord hit alerts with game-only images`.
- Design-artifact PR: `Design Discord hit alerts with game-only images`, from `z-design/discord-hit-alerts` to `main`.
Push `prototype/discord-window-capture` as an evidence-only branch. Do not open a prototype PR or merge either branch here.
The artifact PR contains only CONTEXT.md, this specification, `docs/design/discord-hit-alerts-ticket.md`, the capture ADR, and dated research.
The ticket document is the self-contained approved issue body, not a second specification.
Use native GitHub parent/sub-issue linkage; if unavailable, add explicit reciprocal parent/sub-issue sections.

Ensure these labels exist without replacing unrelated labels:
...
Parent labels: z-design, z-design:spec; add z-design:complete only after all artifacts and links are verified.
Ticket labels: z-design, z-design:ticket, ready-for-agent.
Artifact PR labels: z-design, z-design:artifact. Never put ready-for-agent on the parent or artifact PR.
